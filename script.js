    ...
    cardsData.forEach((card, index) => {
        ...
        cardEl.addEventListener('click', () => {
            ...
            if (count === totalCards) {
                ...
                missionBtn.classList.add('pulse-animation'); 
            } else {
   ...
   function selectChoice(scores) {
    ...
    setTimeout(() => {
        ...
        if (currentQuestionIndex < questions.length) {
          ...
          pages.survey.style.display = 'none';
            if(pages.loading) pages.loading.style.display = 'flex';
            setTimeout(() => {
                if(pages.loading) pages.loading.style.display = 'none';
                showResults();
            }, 2000);
        }
    }, 300); 
...
if (introChartCanvas) {
    ...
    drawChart(introChartCanvas, initialData, true); 
